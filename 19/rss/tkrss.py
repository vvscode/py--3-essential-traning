...
class mainWindow(tkinter.Frame):
    ...
    def go(self, url=None):
        ...
        try:
            ...
            self.errorBox(e, contentWindow.destroy)
    ...
    def listGo(self):
        try:
            ...
            self.errorBox('No feed selected')
        else:
            rec = self._db.get_by_id(self._db_index[int(recno)])
            self.varURL.set('')
            self.go(rec['url'])
    ...
    def listDel(self):
        ...
        self._db.del_by_id(self._db_index[int(recno)])
        self.fillListBox()
